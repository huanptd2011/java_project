...
public class EditProductForm extends JFrame {
	...
	public EditProductForm(Products product, Connection con) {
...
//		        ProductController productController = new ProductController(); // hoặc bạn truyền từ ngoài vào
		        boolean success = productController.updateProduct(product1);

		        if (success) {
		            JOptionPane.showMessageDialog(null, "Chỉnh sửa sản phẩm thành công.");
		            setVisible(false); // đóng form sau khi thêm
		        } else {
		            JOptionPane.showMessageDialog(null, "Chỉnh sửa sản phẩm thất bại.");
		        }
		    }
		});
